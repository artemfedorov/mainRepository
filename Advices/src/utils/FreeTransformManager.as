package com.looksLike.utils {
    ...
    protected var _dragArea:Rectangle;
		public function get dragArea():Rectangle { return _dragArea; }
		public function set dragArea(r:Rectangle):void { _dragArea = r; updateForDragArea(); }
    ...
    // Visual style properties
		...
		public var _handleFillColor:uint = 0xFFF333;
		...
		public function activateSprite(sourceSprite:DisplayObject, config:Object = null):void {
			...
			/* Always conform handler to target. In case target has been updated in other locations.
		...
		protected function conformToSprite():void {
			...
			trace('dispObj.x y=(' + dispObj.x + ',' + dispObj.y + ')' );
			_storedX = dispObj.x;
			_storedY = dispObj.y;
			...
			// Define new LocalCenter
			
			//Get registration point to offset the _localCenter
			var bounds:Rectangle = dispObj.getBounds(dispObj.parent);
			var currentRegX:Number = dispObj.x - bounds.left;
			var currentRegY:Number = dispObj.y - bounds.top;
			//trace("currConfig.reg=" + currConfig.regX + "," + currConfig.regY);
			//trace("currentReg=" + currentRegX + "," + currentRegY);
			
			
			//Save registration point into config
			currConfig = _configs[dispObj] as Object;
			currConfig.regX = currentRegX;
			currConfig.regY = currentRegY;
			_configs[dispObj] = currConfig;
			
			
			//this works for top-left reg points
			//_localCenter = new Point(dispObj.width / 2, dispObj.height / 2);
			
			//this works for center reg points
			//_localCenter = new Point(0,0);
			
			//now this works for both by taking the reg point coordinates into consideration
			_localCenter = new Point((dispObj.width / 2)-currentRegX, (dispObj.height / 2)-currentRegY);
		...
		*/
		...
		protected function updateHandles():void {
			...
			if (dispObj){
				...
				currConfig = _configs[dispObj] as Object;
				//trace("currConfig.reg = " + currConfig.regX + "," + currConfig.regY);				
				regX = currConfig.regX;
				regY = currConfig.regY;
			}
			...
			tempPoint = me.globalToLocal(transformContainer.localToGlobal(new Point(-regX, -regY)));
			...
			tempPoint = me.globalToLocal(transformContainer.localToGlobal(new Point(_boundingRect.width-regX,-regY)));
			...
			tempPoint = me.globalToLocal(transformContainer.localToGlobal(new Point(_boundingRect.width-regX,_boundingRect.height-regY)));
			...
			tempPoint = me.globalToLocal(transformContainer.localToGlobal(new Point(-regX, _boundingRect.height-regY)));
			_blHandler.x = tempPoint.x;
			_blHandler.y = tempPoint.y;
			_blHandler.rotation = _storedRotation / (Math.PI / 180);
			_border.graphics.lineTo(tempPoint.x, tempPoint.y);
			
			//top left to close the loop
			//tempPoint = me.globalToLocal(transformContainer.localToGlobal(new Point(0, 0)));
			tempPoint = me.globalToLocal(transformContainer.localToGlobal(new Point(-regX,-regY)));
			_border.graphics.lineTo(tempPoint.x, tempPoint.y);
		...
		protected function onDispObjMDown(evt:MouseEvent):void {
			...
			trace('mustReactivate=' + mustReactivate);
			if (mustReactivate) {
				activateSprite(dispObj);
			}
		...
		protected function doTranslate(dX:Number, dY:Number):void {
			...
			// Constrain Transform by dragArea
      if (dragArea){
        if (_storedX < dragArea.left) _storedX = dragArea.left;
        if (_storedX > dragArea.right) _storedX = dragArea.right;
        if (_storedY < dragArea.top)_storedY = dragArea.top;
        if (_storedY > dragArea.bottom) _storedY = dragArea.bottom;
      }
		...
		public function setPos(x:Number, y:Number):void {
			...
			// Constrain Transform by dragArea
      if (dragArea){
        if (x < dragArea.left) x = dragArea.left;
        if (x > dragArea.right) x = dragArea.right;
        if (y < dragArea.top) y = dragArea.top;
        if (y > dragArea.bottom) y = dragArea.bottom;
      }
    ...
    public function updateForDragArea():void{
      //TODO: Update so that dragArea is respected
